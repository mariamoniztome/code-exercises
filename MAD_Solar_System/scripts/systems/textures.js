...
function updateProceduralTextures(i) {
  ...
  switch (i) {
    ...
    // ---------------------------------------------------------
    // 0 — Círculos concêntricos animados
    // ---------------------------------------------------------
    case 0: {
    ...
    case 2: {
      ...
      for (let k = 0; k < 12; k++) {
        ...
        for (let x = 0; x <= TEX_SIZE; x += 5) {
          let y = TEX_SIZE / 2 + sin(x * 0.02 + time + k * 0.5) * (15 + k * 3);
          pg.vertex(x, y);
        }
        pg.endShape();
      }
    ...
    case 3: {
      ...
      for (let x = 20; x < TEX_SIZE; x += 40) {
        for (let y = 20; y < TEX_SIZE; y += 40) {
          ...
          let col = (x + y) % 80 < 40 ? color(255, 69, 0) : color(255, 215, 0);
          pg.fill(col);
          pg.circle(x, y, size);
        }
      }
    ...
    // ---------------------------------------------------------
    // 8 — Triângulo Penrose impossível
    // ---------------------------------------------------------
    case 8: {
